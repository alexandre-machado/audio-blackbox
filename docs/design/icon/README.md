...
### Issue #60: the preview renders themselves were wrong, not the artwork

`@rev` found on PR #58 that every preview above, plus `ic_store_candidateA_not_shipped_512.png`, silently dropped the accent band's groove: the "one-time build script" mentioned above was never committed, and it built each SVG `<path>` from the VectorDrawable's `pathData` without carrying `android:fillType="evenOdd"` across as `fill-rule="evenodd"`. SVG defaults to `fill-rule="nonzero"`, so the hole-cutout subpath that punches the groove through the capsule unioned solid instead, and the transparent sliver on each side of the accent band vanished from every render even though the underlying VectorDrawable geometry was (and is) correct — `@rev` confirmed this independently with a from-scratch `cairosvg` reconstruction.

Fixed in `scripts/design/render_icon_previews.py`, now actually committed instead of run once and discarded, so this pipeline is reproducible rather than a repeat of the same failure mode. It maps `android:fillType` to SVG `fill-rule` explicitly and, on every run, samples pixels across the accent band and raises loudly if the groove's transparent gap isn't found before writing any PNG — the same check `@rev` did by hand, now automated. Regenerating every preview under `docs/design/icon/previews/` and the store PNG at `docs/design/store/ic_store_candidateA_not_shipped_512.png` with the corrected pipeline made the groove reappear with no change to any shipped VectorDrawable XML; the geometry was never the problem. Candidate B's foreground/monochrome XML, no longer present under `app/src/main/res/drawable/` since issue #75 moved the shipped foreground to a raster PNG, is restored under `docs/design/icon/candidates/candidateB_*_historical_not_shipped.xml` (extracted from commit `68e63b9`) purely so the `candidateB_shipped_*.png` previews — kept as documented prior art — can be regenerated from a committed source instead of an unreachable one.
