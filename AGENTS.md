...
- **Live-region semantics** ([#66](https://github.com/alexandre-machado/audio-blackbox/issues/66), [#73](https://github.com/alexandre-machado/audio-blackbox/issues/73)): state changes that matter for accessibility are exposed via Compose semantics live regions, not silently updated visuals only.
  - **Open gap, partially addressed: the home-screen widget's proactive announcement path is attempted but UNVERIFIED ([#279](https://github.com/alexandre-machado/audio-blackbox/issues/279), raised as an advisory finding by `@rev` in both round 2 and round 4 of PR [#278](https://github.com/alexandre-machado/audio-blackbox/pull/278); corrected on PR [#289](https://github.com/alexandre-machado/audio-blackbox/pull/289) review after an earlier revision of this entry wrongly asserted the gap was closed by API-surface analysis alone — see that PR's `@rev` review for the correction).** `RecordingWidgetProvider`/`RecordingWidgetRenderer` paint the home-screen widget through `RemoteViews`, which has no direct equivalent of Compose's `liveRegion` semantics — but the gap is narrower than a first pass concluded:
    - `RemoteViews` exposes no dedicated `setAccessibilityLiveRegion`/`sendAccessibilityEvent`/`announceForAccessibility` *method of its own*. But `RemoteViews.setInt(viewId, methodName, value)` is a **generic, by-name reflective dispatcher**: it invokes any public single-`int`-arg method on the real host `View` at apply time, in the launcher's process. `RecordingWidgetRenderer.render` already exploited exactly this mechanism before #279 (`views.setInt(R.id.widget_annunciator, "setColorFilter", ...)`, see `widget_annunciator_dot.xml`), and `View.setAccessibilityLiveRegion(int)` has the identical shape (public, one `int` argument), so the same dispatcher reaches it. **This app's code now attempts it**: `RecordingWidgetRenderer.render` calls `views.setInt(R.id.widget_root, "setAccessibilityLiveRegion", View.ACCESSIBILITY_LIVE_REGION_POLITE)` on every repaint, alongside the existing per-state `setContentDescription` on that same node (see [`RecordingWidgetRendererTest`](app/src/test/java/cc/machado/audioblackbox/widget/RecordingWidgetRendererTest.kt), which asserts this specific call is issued).
    - The widget provider (`RecordingWidgetRenderer.render`) never holds a reference to the real `View` instances rendering the widget on screen — those live in the launcher/host process, not this app's process — so there is no object this app's own code could call `View.sendAccessibilityEvent()`/`announceForAccessibility()` on directly; the `setInt` reflective route above is the only mechanism available to reach the host's `View` API surface at all.
    - **What is UNVERIFIED, plainly stated as such**: whether a real launcher's `AppWidgetHostView` actually turns the live-region flag plus a `contentDescription` change into an audible TalkBack announcement while the widget is on screen but not focused. That is an on-device, real-launcher, real-TalkBack observation — a Tier 2 (physical device) concern per §6 — and no JVM/Robolectric tier can answer it. It has not been observed on hardware as of this entry; do not read the code change above as proof it works, only as proof the call is now attempted. Confirming it (or ruling it out) on the S25 with TalkBack enabled via `scripts/device-smoke.sh` is an open follow-up.
