...
- **Live-region semantics** ([#66](https://github.com/alexandre-machado/audio-blackbox/issues/66), [#73](https://github.com/alexandre-machado/audio-blackbox/issues/73)): state changes that matter for accessibility are exposed via Compose semantics live regions, not silently updated visuals only.
  - **Named exception: the home-screen widget cannot satisfy this rule ([#279](https://github.com/alexandre-machado/audio-blackbox/issues/279), raised as an advisory finding by `@rev` in both round 2 and round 4 of PR [#278](https://github.com/alexandre-machado/audio-blackbox/pull/278)).** `RecordingWidgetProvider`/`RecordingWidgetRenderer` paint the home-screen widget through `RemoteViews`, which has no equivalent of Compose's `liveRegion` semantics:
    - `RemoteViews` exposes no `setAccessibilityLiveRegion`-style call, no `sendAccessibilityEvent`, and no `announceForAccessibility` — it can only set static view properties (text, image, visibility, `contentDescription`) that are replayed reflectively in the host launcher's process.
    - The widget provider (`RecordingWidgetRenderer.render`) never holds a reference to the real `View` instances rendering the widget on screen — those live in the launcher/host process, not this app's process — so there is no object this app could call `View.sendAccessibilityEvent()`/`announceForAccessibility()` on even if it wanted to proactively announce a transition.
    - Calling `RemoteViews.setContentDescription` again on every repaint (already done for `widget_root` and `widget_action_button`, unchanged by this entry) only updates the description a TalkBack user reads *if and when they navigate to or already hold focus on that node*; whether the host launcher's `AppWidgetHostView` turns that into a proactive "content changed" announcement while the widget is on screen but not focused is a property of the launcher implementation, not something this app's code controls or can force.
    - **This is recorded as a verified API-surface limitation, not an oversight**: there is no `RemoteViews`/`AppWidgetProvider` API this app could adopt to close the gap from its own process. What remains open, and is explicitly *not* claimed as verified here, is the on-device, real-launcher, real-TalkBack observation of exactly what (if anything) gets announced when the widget's state changes while on screen — that is a Tier 2 (physical device) observation per §6, and is tracked as a follow-up device-smoke step rather than asserted from the API surface alone.
    - Do not treat this exception as license to skip the existing per-state `contentDescription`s — those remain the *only* affordance a TalkBack user has for this widget (see [`RecordingWidgetStateMapperTest`](app/src/test/java/cc/machado/audioblackbox/widget/RecordingWidgetStateMapperTest.kt) for the regression coverage that each `CaptureState` maps to its own correct, distinct root and action-button description).
