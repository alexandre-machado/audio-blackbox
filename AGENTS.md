...
- **Iconography ([#220](https://github.com/alexandre-machado/audio-blackbox/issues/220), supersedes the strict reading of PR [#74](https://github.com/alexandre-machado/audio-blackbox/pull/74))**:
  ...
  - **Scoped exception for `ic_tile_logo.xml` ([#268](https://github.com/alexandre-machado/audio-blackbox/issues/268), narrow carve-out, supersedes nothing above — it does not weaken the "no genuinely novel glyphs" line for anything else) (origin: #268 · 2026-09-01):** the Quick Settings tile icon was first attempted as a traced-down reduction of `ic_launcher_monochrome.xml`'s isometric illustration to a 24dp silhouette (PR [#269](https://github.com/alexandre-machado/audio-blackbox/pull/269) first revision) and rejected on real-device evidence — reviewed on the owner's S25, it read as an unrecognisable, off-centre mass, because the app logo's recognisability lives in colour/lettering/internal illustration detail that cannot survive reduction to a single-colour 24dp silhouette, and reducing by largest-area subpath kept the wrong information regardless of care taken. The owner granted a scoped exception permitting one **purpose-built, novel 24dp pictogram** (a crash-survivable-recorder canister shape, not traced from any existing asset) for this single drawable only. Limits: it must still be a single-colour, system-tintable silhouette on a transparent 24dp ground (no groups, no gradients, no hardcoded brand colour); it is not a licence for a decorative/illustrative icon, a third-party icon pack, or any other surface — every other icon in the repo stays under the stock-Material-shapes rule above.
