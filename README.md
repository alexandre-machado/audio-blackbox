...
- 🎯 **Two Primary Capture Modes**:
  - **Save Recent Past (Lookback)**: Instantly snapshot everything currently buffered from the memory ring buffer into an AAC (`.m4a`) file. One action, always the whole buffer -- the old fixed-window selector was retired in #121 because it promised windows the buffer might not hold. Since #410 a successful save also advances the buffer's export floor, so the next save starts where the previous file ended and consecutive saves never overlap.
...
3. **The 2x Re-allocation Cost, Guarded**: Growing an array in Kotlin means allocating the new, larger array while the old one is still alive. Stretching a 100 MB buffer to 150 MB briefly needs 250 MB. `RingBuffer.resize` (#223) does exactly this copy when you change the window or preset during capture, keeping the audio already buffered, so it first checks an injected `MemoryBudget` and refuses (`ResizeOutcome.Refused`, nothing allocated, the current buffer untouched) when the projected peak would not fit (#272). A refusal surfaces as a visible "Setting not applied" error, never a crash and never a silently changed setting.
