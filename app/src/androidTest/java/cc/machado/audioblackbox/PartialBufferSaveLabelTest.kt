 ...
 *
 * The oracle: while under a minute is buffered, the exported filename must embed `_0min` (floored,
 * never rounded up to `_1min`), and the notification's Save action text must NOT be the old
 * misleading "Save last 0 min" -- it must be the honest seconds-based copy instead.
 */
...
class PartialBufferSaveLabelTest {
    ...
    fun subMinuteBuffer_exportedFileAndNotificationLabel_agreeAndNeverOverstate() {
        ...
        checkNotNull(row)
        assertTrue(
            "a sub-minute buffer must export a file labeled _0min (floored, never rounded up) -- " +
                "got name: $row",
            row.contains("_0min."),
        )
    }
    ...
    private fun pollForExportedRow(sinceMillis: Long, timeoutMillis: Long): String? {
